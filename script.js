...
function getFieldRows(book) {
  return [
    ...
    ["Quantità", clean(getField(book, "QUANTITA", "Quantita", "Quantità", "quantita", "quantità"))],
...
function createNewBookCard(book) {
  ...
  li.className = "new-book-item";
  li.innerHTML = `
    <button class="new-book-button" type="button" data-book-id="${escapeHtml(bookId)}">
      <span class="new-book-cover">
        ${createCoverMarkup(book, cover)}
      </span>

      <span class="new-book-info">
        <strong>${title}</strong>
        <span>${author}</span>
      </span>
    </button>
  ...
  li.querySelector(".new-book-button").addEventListener("click", function () {
    openBookModal(bookId);
  });
...
function renderNewBooks() {
  ...
  const latestBooks = books.slice(-10).reverse();

  if (latestBooks.length === 0) {
    newBooksList.innerHTML = `<li class="new-books-loading">Nessuna novità disponibile.</li>`;
    return;
  }

  latestBooks.forEach((book) => {
    newBooksList.appendChild(createNewBookCard(book));
  });
}
...
function showLoadingError() {
  ...
  if (newBooksList) {
    newBooksList.innerHTML =
      `<li class="new-books-loading">Novità non disponibili.</li>`;
  }
}
...
function parseGoogleSheetValue(cell) {
  if (!cell) return "";
  return cell.f || cell.v || "";
}

function parseGoogleSheetJson(table) {
  const headers = table.cols.map((col, index) => clean(col.label, `Colonna ${index + 1}`));

  return table.rows.map((row) => {
    const book = {};

    headers.forEach((header, index) => {
      book[header] = parseGoogleSheetValue(row.c[index]);
    });
...
function loadBooksWithGoogleSheetScript(fallback) {
  const callbackName = `initializeBiblioLeoCatalog${Date.now()}`;
  ...
  window[callbackName] = function (response) {
    ...
    initializeCatalog(parseGoogleSheetJson(response.table));
  };
  ...
  script.src =
    sheetURL.replace("/pub?output=csv", "/gviz/tq?tqx=responseHandler:" + callbackName);
...
function loadBooksWithPapa() {
  ...
  Papa.parse(sheetURL, {
    ...
    error: function (error) {
      console.error(error);
      loadBooksWithGoogleSheetScript();
    },
  });
}
...
function loadBooks() {
  ...
  if (newBooksList) {
    newBooksList.innerHTML = `<li class="new-books-loading">Caricamento novità...</li>`;
  }

  loadBooksWithGoogleSheetScript(loadBooksWithFetch);
}
